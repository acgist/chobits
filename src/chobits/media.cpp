...
bool chobits::media::open_media() {
    ...
    std::vector<torch::Tensor>& video = dataset.video;
...
std::tuple<bool, at::Tensor, at::Tensor> chobits::media::get_data() {
    torch::Tensor audio;
    torch::Tensor video;
    {
        std::unique_lock<std::mutex> lock(dataset.mutex);
        ...
        audio = dataset.audio.front();
        video = dataset.video.front();
        dataset.audio.erase(dataset.audio.begin());
        dataset.video.erase(dataset.video.begin());
    }
