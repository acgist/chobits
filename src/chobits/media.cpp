...
static AVPixelFormat video_pix_format = AV_PIX_FMT_RGB24;

static AVSampleFormat  audio_format           = AV_SAMPLE_FMT_S16;
static AVChannelLayout audio_layout           = AV_CHANNEL_LAYOUT_MONO;
static int             audio_sample_rate      = 48000;
static int             audio_nb_channels      = audio_layout.nb_channels;
static int             audio_bytes_per_sample = av_get_bytes_per_sample(audio_format);
    ...
    while(player.running && av_read_frame(format_ctx, packet) == 0) {
        if(packet->stream_index == audio_index) {
            ...
            while(player.running && avcodec_receive_frame(video_codec_ctx, frame) == 0) {
                ...
                #ifdef MEDIA_PREVIEW
                // 正常速度
                uint64_t time_base = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
                int delay = (int) (video_time * 1000 - (time_base - base_time));
                if(delay > 0) {
                    SDL_Delay(delay);
                }
                #endif
...
bool chobits::media::open_hardware() {
    ...
    if(ret != 0) {
        ...
        avformat_close_input(&audio_format_ctx);
        avformat_close_input(&video_format_ctx);
        std::printf("打开音频硬件失败：%d - %s", ret, video_device_name.c_str());
        return false;
    }
    av_dict_free(&audio_options);
    av_dict_free(&video_options);
    ...
    std::thread audio_thread([audio_format_ctx, &audio_frame_count]() {
        ...
        int ret = avcodec_open2(audio_codec_ctx, audio_codec, nullptr);
        if(ret != 0) {
            avcodec_free_context(&audio_codec_ctx);
            std::printf("打开音频解码器失败：%d\n", ret);
            return;
...
bool chobits::media::play() {
    ...
    SDL_OpenAudio(&player.audio, nullptr);
    SDL_PauseAudio(0);
    SDL_Event event;
    while(player.running) {
...
static bool audio_to_tensor(SwrContext* swr, AVFrame* frame) {
    ...
    int pos  = player.audio_buffer.size();
    int size = audio_nb_channels * audio_bytes_per_sample * frame->nb_samples;
    player.audio_buffer.resize(pos + size);
    uint8_t* buffer = player.audio_buffer.data() + pos;
...
static bool video_to_tensor(SwsContext* sws, AVFrame* frame) {
    ...
    if(player.running) {
        ...
        SDL_UpdateTexture(player.texture, nullptr, buffer, width);
...
static void audio_callback(void* userdata, Uint8* stream, int len) {
    if(player.audio_buffer.size() < len) {
        return;
    }
    std::lock_guard<std::mutex> lock(player.mutex);
    std::memcpy(stream, player.audio_buffer.data(), len);
    player.audio_buffer.erase(player.audio_buffer.begin(), player.audio_buffer.begin() + len);
}
